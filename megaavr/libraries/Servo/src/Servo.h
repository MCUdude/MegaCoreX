...
typedef struct  {
  ...
  // optimization it supports (losing digitalWrite() in the ISR) has surprisingly little impact - though I do think it has a more significant impact on time spent
  // in the ISR, which you don't see most of the time...
...
class Servo {
  public:
    ...
    uint8_t attach(uint8_t pin, int16_t min, int16_t max); // as above but also sets min and max values for writes.
    void detach();
